...
<BaseLayout title={$t('projects.not-found.page.title')}>
    ...
    {#if langs.length}
        <ul>
            {#each langs as lang}
                <li><A href="/{data.available_lang[lang]}">{$t(`lang.${lang}`)}</A></li>
            {/each}
        </ul>
    {/if}
    <div class="text-lg">
    <A href={'/' + data.backPath || '/'}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 inline">
